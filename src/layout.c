...
apply_inline_layout_attrs(NsPangoAttrList *attrs, const ns_box *box)
{
    ...
    for (gint ii = (gint)box->attrs->len - 1; ii >= 0; ii--) {
        ...
        case NS_INLINE_FONT_WEIGHT:
            a = ns_pango_attr_weight_new(ns_paint_pango_weight(r->font_weight));
            break;
                              ...
                              double *cursor_y)
{
    ...
    NsPangoLayout *layout = inline_box_fragment_layout(src, col_w,
                                                     child_inherited);
                          ...
                          double run_width, const ns_style *style)
{
    ...
    NsPangoLayout *layout = inline_box_fragment_layout(run, run_width, style);
    int line_count = ns_pango_layout_get_line_count(layout);
    ...
    NsPangoLayoutLine *ln = ns_pango_layout_get_line_readonly(layout, tail_line);
...
box_hit_stack_walk(const ns_box *root, double x, double y, GArray *hits)
{
    ...
    if (root->inline_atomics)
        for (guint i = 0; i < root->inline_atomics->len; i++) {
            ...
            double ax, ay;
            inline_atomic_hit_point(root, atomic, cx, cy, &ax, &ay);
            box_hit_stack_walk(atomic->box, ax, ay, hits);
        }
}
...
static gboolean
hit_node_refines(const ns_node *candidate, const ns_node *current)
{
    if (!candidate || !current) return FALSE;
    for (const ns_node *n = candidate; n; n = n->parent)
